...
当前项目根目录检出的 `local/aggregate` 是用于本地体验的集成分支。默认基线是最新匹配的 `dsh-v*` 稳定 tag（上游均为 prerelease，取最成熟的 rc），不是 `origin/master` 尖端。`origin/master` 上未进该 tag 的提交视为未发布债务，除非用户明确选择跟主干。不要从聚合分支创建上游 PR。验证后的聚合提交必须推送到个人 fork 的 `local/aggregate`，以便另一环境可直接取得相同的已聚合源码；不得推送到上游。

当前纳入的独立改动如下：

| 独立分支 | 上次打包源提交 | 上次打包聚合提交 | 上游回馈 | 目的 |
| --- | --- | --- | --- | --- |
| `fix/acp-mid-turn-steering` | `11be52659` | `6a83e28ce` | [hxy91819/deepseek-harness#1](https://github.com/hxy91819/deepseek-harness/issues/1) | ACP adapter 把并发 session/prompt 作为原生 mid-turn steering 接入，并宣告 `_meta.midTurnSteering` |
...
3. 独立 worktree 负责开发、测试、提交和发布；聚合分支只通过 `git cherry-pick -x <commit>` 引入已验证提交。已完成且已验证的 source 分支和 `local/aggregate` 均须发布到个人 fork；rebase 或重建后的 ref 可直接使用带明确 lease 的 `--force-with-lease`，无需再次请求授权，但不得 force-push 上游。
...
5. 上游同步先检查新增分支、源提交变化、上游变化及每个回馈 issue 的采纳信号，向用户呈现维护建议并请求决定。默认把受影响的 feature/fix 分支 rebase 到最新匹配的 `dsh-v*` 稳定 tag 并验证，再按该 tag 重建聚合；`origin/master` 上未进 tag 的提交只作为债务报告。若用户明确不 rebase，可以继续以当前本地基线增量打包，并保留未同步上游的状态。
6. 聚合层出现问题时，优先在相应独立分支修复并以新的提交重新引入；不要在聚合分支写无法回流的产品代码。
